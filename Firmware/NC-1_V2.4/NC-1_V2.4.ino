...
void CheckAutoMode(){  
 if(AutoMode == digitalRead(ColorModePin)){
   memset(OldPattern,0,sizeof(OldPattern)); 
   memset(PatternValue,0,sizeof(PatternValue)); 
   memset(patternArray,0,sizeof(patternArray)); 
   setOutputArray();
...
void AutoPattern(){ 
 ...
 if(PatternValue[AutoPatternNumber] == 0){
    AutoPatternNumber +=1;
    if(AutoPatternNumber >= 10){
      AutoPatternNumber = 0;
    }
  }else{
    currentPattern = AutoPatternNumber;
    Mode1[AutoPatternNumber]();
  }
  AutoPatternTime ++;
  if(AutoPatternTime >= MaxAutoTime){
    AutoPatternNumber += 1;
    AutoPatternTime = 0;
    if(AutoPatternNumber >= 10){
      AutoPatternNumber = 0;
